# I55 — repair aba CUSTOS: memorial, headers 6 cols, formatos, protecao
param(
...
$params = @{
  action = "repararCustosPlanilhaAdmin"
  adminPin = $AdminPin
}
...
if ($out.dryRun) {
  ...
  Write-Host "Repair OK - linhas=$($out.formatos.linhas) schemaOk=$($out.schemaOk)" -ForegroundColor Green
...
$cusSchema = $schema.resultado.CUSTOS
$color = if ($cusSchema.ok) { "Green" } else { "Yellow" }
Write-Host "validarSchema CUSTOS ok=$($cusSchema.ok) schemaOk=$($schema.schemaOk) versao=$($schema.versao)" -ForegroundColor $color
