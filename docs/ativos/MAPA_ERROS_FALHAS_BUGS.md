...
| **I20** | **Col C no cadastro + `serverTs` na resposta API (latência ~27s)** | **Timer adiantado ao ▶ (ex. 09:33 em plano 10 min); horaInicio HH:mm mascara atraso** | GAS **v1.5.66**: col Y = `clientTs` (clique) se drift ≤2min; FE **v1.7.77**: início otimista no clique | `guard.gas.salvar.horaVazia`; `iniciarTimer.clientTs`; FE `iniciarContagem` otimista | Pendente **10:00** → ▶ mostra **10:00**; portal = balcão; **tablet** |
