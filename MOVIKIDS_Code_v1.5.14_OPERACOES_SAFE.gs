// ═══════════════════════════════════════════════════════════
// MOVI KIDS — Google Apps Script v1.5.11
...
// ── VEÍCULOS VÁLIDOS — v1.5.3: Triciclo 01 adicionado ───────
...
function ping_() {
  ...
  return resp_({
    ...
    sistema: 'MOVI KIDS v1.5.13_SAFE'
  });
}
  ...
  dados.forEach((r, idx) => {
    if (String(r[14]).trim() === 'Ativa') {
      ...
      ativas.push({
        ...
        id:              r[0],
        data:            cellToStr_(r[1]),
        horaInicio:      ts ? fmtHoraLocal_(new Date(ts)) : cellToStr_(r[2]),
        startTimestamp:  ts,
        tipo,
        plano,
        veiculo,
...
function locacaoObj_(row, rowIndex) {
  return {
    ...
    status: String(row[14] || ''), veiculo: String(row[15] || ''), pagamento: String(row[16] || ''), observacao: String(row[17] || ''),
    startTimestamp: Number(row[24] || 0), extendedMins: Number(row[25] || 0), extendedValor: Number(row[26] || 0)
  };
}
...
function editarLocacao_(p) {
  ...
  try {
    ...
    const started = Number(row[24] || 0) > 0;
...
function carregarInicio_() {
  ...
  const cacheKey  = 'carregarInicio_v2';
  ...
  try { cache.put(cacheKey, JSON.stringify(resultado), 20); } catch(e) {}
  return resultado;
}
...
function fbDadosSessao_(row, status, rowIndex) {
  ...
  return {
    ...
    plano:          String(row[5]  || ''),
    mins:           Number(row[6]  || 0),
    adicionalPorMin:Number(row[7]  || 1),
    ...
    startTimestamp: Number(row[24] || 0),
