...
$co = Invoke-Mk @{ action = 'comandoOperacional'; adminPin = '1416'; authRole = 'admin' }
...
$ops = Invoke-Mk @{ action = 'listarOperadores'; adminPin = '1416' }
Write-Host '--- listarOperadores.sessaoAtiva ---'
Write-Host ($ops.sessaoAtiva | ConvertTo-Json -Compress)

$gp = Invoke-Mk @{ action = 'gpPainelAdmin'; adminPin = '1416'; authRole = 'admin' }
...
$ponto = Invoke-Mk @{ action = 'gpAlertasPonto'; adminPin = '1416'; authRole = 'admin' }
Write-Host '--- gpAlertasPonto ---'
Write-Host "total=$($ponto.total)"
foreach ($a in @($ponto.alertas)) { Write-Host "  $($a.nome): $($a.mensagem)" }

$res = Invoke-Mk @{ action = 'resumoDia'; adminPin = '1416'; authRole = 'admin'; data = (Get-Date -Format 'dd/MM/yyyy') }
