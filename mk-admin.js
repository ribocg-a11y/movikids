...
/** I157d: gráfico = Base × Real × Ritmo (3 linhas). Ritmo = reta ritmoAtual×dia. */
function renderReceitaMesChart_(d) {
  ...
  // I157: ritmo no gráfico = ritmo ATUAL × dia (reta). Base = pro-rata. Real = soma.
  ...
  setText2('nk-receita-mes-label',
    ...
    + ' · base ' + R2(baseHoje)
    + ' · ritmo ' + R2(ritmoHoje)
    + (ritmoDiaria > 0 ? (' (' + R2(ritmoDiaria) + '/dia · dias ' + refLbl + ')') : '')
  ...
  chartReceitaMes = new Chart(cv, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: {
          // I157b: um balão compacto (3 linhas), não 3 séries empilhadas
          ...
          callbacks: {
            ...
            label: function(ctx) {
              ...
              return [
                'Real ' + R2(real),
                'Base ' + R2(bA) + '  ·  Δ ' + (dB >= 0 ? '+' : '') + R2(dB),
                'Ritmo ' + R2(rA) + '  ·  Δ ' + (dR >= 0 ? '+' : '') + R2(dR)
              ];
            }
          }
        }
  ...
  if (ins) {
    ...
    if (ritmoHoje > 0 && ritmoDiaria > 0) {
      if (diffRitmoHoje >= 0) {
        msgs.push('Ritmo 3d (' + refLbl + '): ' + R2(ritmoDiaria) + '/dia — real no ritmo ou acima.');
      } else {
        msgs.push('Ritmo 3d (' + refLbl + '): ' + R2(ritmoDiaria) + '/dia — real '
          + Math.abs(pctRitmoHoje) + '% abaixo do termômetro (dias fracos no início puxam a média).');
      }
      msgs.push('Mantendo → ~' + R2(ritmoMes) + ' no mês.');
    }
    if (baseMes > 0) {
      msgs.push('Piso DRE do mês: ' + R2(baseMes)
