// ═══════════════════════════════════════════════════════════
// MOVI KIDS — Google Apps Script v1.5.23
...
function carregarOperacaoConfig_() {
  try {
    return resp_({ versao: 'v1.5.23', config: operacaoConfig_() });
  } catch(ex) {
...
// ── RELACIONAMENTO / RESPONSAVEIS (v1.5.23) ──────────────────
...
function listarResponsaveis_(p) {
  try {
    ...
    const shLoc = sh_(SH_LOC);
    const last = shLoc.getLastRow();
    if (last < DATA_ROW) return resp_({ responsaveis: [], total: 0 });
