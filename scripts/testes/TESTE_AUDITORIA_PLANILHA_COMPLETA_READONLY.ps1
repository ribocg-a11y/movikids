...
try {
  ...
  try {
    $diag = Invoke-MkApi @{ action = "diagnosticoPlanilhaCompletoAdmin"; adminPin = $AdminPin; _t = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds() }
  ...
  if ($fail.Count -gt 0) {
    ...
    $result.summary = "Auditoria OK; $($warn.Count) avisos"
  } else {
    $result.status = "ok"
