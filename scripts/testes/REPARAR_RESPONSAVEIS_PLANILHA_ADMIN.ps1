# I59 — repair aba RESPONSAVEIS: header 9 cols, formatos CRM (layout legado L1)
param(
...
if ($out.dryRun) {
  Write-Host "DRY RUN - schemaOk=$($out.schema.ok) cadastros=$($out.audit.cadastrosPlanilha) problemas=$($out.audit.problemas.Count)" -ForegroundColor Yellow
} else {
  Write-Host "Repair OK - linhas=$($out.formatos.linhas) cadastros=$($out.cadastros) schemaOk=$($out.schemaOk)" -ForegroundColor Green
...
$respSchema = $schema.resultado.RESPONSAVEIS
$color = if ($respSchema.ok) { "Green" } else { "Yellow" }
Write-Host "validarSchema RESPONSAVEIS ok=$($respSchema.ok) schemaOk=$($schema.schemaOk) versao=$($schema.versao)" -ForegroundColor $color
