// ═══════════════════════════════════════════════════════════
// MOVI KIDS — Google Apps Script v1.5.29
...
// v1.5.29: consulta status SMS (Delivered/Failed) por gatewayId
...
function carregarOperacaoConfig_() {
  try {
    return resp_({ versao: 'v1.5.29', config: operacaoConfig_() });
  } catch(ex) {
...
function smsTextoPortal_(tipo, loc) {
  ...
  if (tipo === 'alerta') {
    return 'MOVI KIDS: o tempo de ' + crianca + ' esta perto de acabar. Acompanhe pelo portal: ' + portal;
  }
  if (tipo === 'esgotado') {
    return 'MOVI KIDS: o tempo de ' + crianca + ' encerrou e extras podem estar contando. Acompanhe: ' + portal;
  }
  if (tipo === 'extensao') {
...
function smsTextoAvulso_(tipo, nome, crianca) {
  ...
  if (tipo === 'retorno') {
    return 'MOVI KIDS: oi, ' + n + '! Que tal uma nova aventura para ' + c + ' hoje? Esperamos voces.';
  }
