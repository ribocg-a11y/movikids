...
| **I158** | **Golden PDF/e-mail contava Cancelada** | `_gerarHtmlRelatorio_` excluía só Ativa → Cancelada inflava fat/CTO; n=linhas ≠ kpiMes contas | GAS **v1.5.222** `isStatusFaturavelCaixa_` + `aggMovimentacaoMesCaixa_` (paridade kpiMes) · `guard.i158.*` | `INCIDENTE_I158_*` · `TESTE_I158_GOLDEN_*` | ⏳ Nova versão Web |
| **I156** | **Linha Ritmo (3 dias) sumiu no Dashboard** | `lastNBillingDays` lia `fatMap[1]` mas mapa usa `"01"` → media 0 → FE não plota | GAS **v1.5.221** `fatMapDiaVal_` · FE **v1.9.115** fallback | `INCIDENTE_I156_*` · `guard.i156.*` | ✅ live · ritmo **785.67**/dia ref 6–8 |
