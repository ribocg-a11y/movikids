# Deploy GAS v1.5.59 — KPI faturamento do ano (Dashboard)
...
| **Ping (deve retornar v1.5.59 após deploy)** | https://script.google.com/macros/s/AKfycbwakQ-_aWsF5lFGLsiwB5UvJ4AlpW88krSv8daPeMvULwX5FOIdMhGVgdGd0G35270Y/exec?action=ping |
| **App (FE v1.7.54+)** | https://ribocg-a11y.github.io/movikids/?force=1.7.54 |
...
- `buscarKPIsAdmin` retorna **`fatAno`** e **`nAno`**: faturamento **acumulado do ano civil inteiro** (todas as locações encerradas naquele ano, ex. **2026**).
...
1. Colar GAS v1.5.59 no Apps Script (mesmo projeto).
2. Salvar → Implantar nova versão no **mesmo** Deploy ID.
3. Abrir **Ping** — deve retornar `"versao":"v1.5.59"`.
4. No app: **Dashboard** → primeiro KPI “Faturamento do ano” (ex.: `2.751 em 2026` com subtexto `148 locações em 2026`).
