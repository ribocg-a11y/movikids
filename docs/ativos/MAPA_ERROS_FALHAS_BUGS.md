...
| **I125** | **Salvar locação / ▶ cronômetro 18s+8s (reincidente)** | `nextId`+`findContaMestre` scan 2k; dash invalidate; N×getRange; **Firebase UrlFetch 2–6s** no path | GAS **v1.5.203** I125+I125b (sem FB/format no salvar/▶) · FE **v1.9.67** sync suave | `guard.i125.*` · **`TESTE_I125_SALVAR_INICIAR_PERF`** | salvar&lt;8s · ▶ API&lt;5s (após Nova versão Web) |
