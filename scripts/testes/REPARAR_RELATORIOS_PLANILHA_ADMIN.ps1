# I60 — repair aba RELATORIOS: header 6 cols, formatos PDFs mensais
param(
...
if ($out.dryRun) {
  Write-Host "DRY RUN - schemaOk=$($out.schema.ok) registros=$($out.audit.registros)" -ForegroundColor Yellow
} else {
  Write-Host "Repair OK - registros=$($out.registros) schemaOk=$($out.schemaOk)" -ForegroundColor Green
}

$schema = Invoke-MkApi @{ action = "validarSchema" }
$relSchema = $schema.resultado.RELATORIOS
$color = if ($relSchema.ok) { "Green" } else { "Yellow" }
Write-Host "validarSchema RELATORIOS ok=$($relSchema.ok) schemaOk=$($schema.schemaOk) versao=$($schema.versao)" -ForegroundColor $color
