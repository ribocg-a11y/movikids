...
| **I125** | **Salvar locação / ▶ cronômetro 18s+8s (reincidente)** | `nextId`+`findContaMestre` scan 2k linhas; `invalidateDashCaches_` em toda escrita; `iniciarTimer` N×getRange; FE `force` frio | GAS **v1.5.202** nextId O(1) · conta mestre lookback dia · invalidate sem dash · timer batch · FE **v1.9.67** sync suave | `guard.i125.*` · **`TESTE_I125_SALVAR_INICIAR_PERF`** | salvar&lt;8s · ▶ API&lt;5s (após Nova versão Web) |
